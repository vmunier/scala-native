...
object IntSuite extends tests.Suite {
  ...
  test("should_support_unary_minus") {
  ...
  test("percent_should_never_produce_a_negative_0_#1984") {
    @noinline def value: Int = -8
    assertTrue((value % 8).asInstanceOf[java.lang.Integer].equals(0))
  }

  test("should_support_shift_left") {
    def test(a: Int, b: Int, expected: Int): Unit =
      assertEquals(expected, a << b)
  ...
  test("should_support_shift_right") {
    def test(a: Int, b: Int, expected: Int): Unit =
      assertEquals(expected, a >> b)
  ...
  test("should_support_shift_right_sign_extend") {
    def test(a: Int, b: Int, expected: Int): Unit =
      assertEquals(expected, a >>> b)
